...
# https://github.com/Chillee/lit-llama/blob/main/model.py#L75
...
class KVCacheAggregator(nn.Module):
    ...
    def initialize(
        self, layers, max_batch_size, max_seq_length, n_heads, head_size, device='cuda', dtype=torch.bfloat16
    ):
        # cache_shape = (max_batch_size, n_heads, max_seq_length, head_size)
        self.kv_caches = nn.ModuleList(
            [KVCache(max_batch_size, max_seq_length, n_heads, head_size) for _ in range(layers)]
        )
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        att = att.masked_fill(self.bias[:, :, :t, :t] == 0, -1e10)
