...
class Tortoise(nn.Module):
    ...
    def generate_speech_tokens(
        ...
        *,
        ...
        t0 = perf_counter()
        torch.cuda.nvtx.range_push("sampling_loop")
        ...
        while True:
            ...
            # repetition penalty
            torch.cuda.nvtx.range_push("rep_penalty")
            ...
            # temperature
            torch.cuda.nvtx.range_push("temperature")
        ...
        t = perf_counter() - t0
        model_size = sum(
            [p.numel() * p.data.element_size() for p in itertools.chain(self.parameters(), self.buffers())]
        )
        print(f"Time for inference : {t:.02f} sec total, {speech_inputs.shape[1] / t:.02f} tokens/sec")
        print(f"Bandwidth achieved: {model_size * (speech_inputs.shape[1]/ t) / 1e9:.02f} GB/s")
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        ...
        conditioning_latent = conditioning_encoder.get_conditioning(speech_samples=[Path("../emma.wav")])
